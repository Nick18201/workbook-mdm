...
def _draw_form_section(c, form, current_y, title, text, input_name, input_height, content_x, content_w):
    current_y = draw_title(c, title, pos=(content_x, current_y), size=16, color=PDFStyle.COLOR_ACCENT_RED)
...
def create_profil_page(c):
    ...
    current_y = draw_title(c, "P1 : PROFIL (Qui je suis)", pos=(content_x, current_y), size=22)
...
def create_parcours_page(c):
    ...
    current_y = draw_title(c, "P2 : PARCOURS (D'où je viens)", pos=(content_x, current_y), size=22)
...
def create_preuves_page(c):
    ...
    current_y = draw_title(c, "P3 : PREUVES (Réalisations)", pos=(content_x, current_y), size=22)
...
def create_potentiel_page(c):
    ...
    current_y = draw_title(c, "P4 : POTENTIEL (Où je vais)", pos=(content_x, current_y), size=22)
