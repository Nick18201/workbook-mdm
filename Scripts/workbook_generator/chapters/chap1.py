...
def create_meteo_page(c):
    ...
    create_input_field(form, 'meteo_emotion_word', pos=(text_x + 5.5*cm, y_opts-5), size=(8*cm, 20),
    ...
    for opt in options:
        create_checkbox(form, f'meteo_{opt.split()[0]}', pos=(opt_x, opt_y), size=0.6*cm, tooltip=opt)
    ...
    for i in range(11):
        ...
        create_checkbox(form, f'meteo_energy_{i}', pos=(x_mark - 0.22*cm, y_energy - 2.1*cm), size=0.45*cm, tooltip=f'Niveau {i}')
    ...
    create_input_field(form, 'meteo_pensee', pos=(text_x, y_thought - 3*cm), size=(width - text_x - 1*cm, 2.5*cm),
...
def create_vision_page(c):
    ...
    for title, dx, dy in axes:
        ...
        create_input_field(form, f'vision_{main_title.strip()}', pos=(f_x, f_y), size=(field_width, field_height), tooltip="Phrase de synthèse", multiline=True, fill_color=PDFStyle.COLOR_CARD_CREME)
...
def create_boussole_page(c):
    ...
    create_input_field(form, 'boussole_enjeu', pos=(text_x, y_goal - 2*cm), size=(width - text_x - 1*cm, 1.5*cm),
                       tooltip='Enjeu principal', multiline=True)
                       
    y_benefit = y_goal - 3*cm
    c.drawString(text_x, y_benefit, "Pour pouvoir :")
    
    create_input_field(form, 'boussole_benefice', pos=(text_x, y_benefit - 2*cm), size=(width - text_x - 1*cm, 1.5*cm),
    ...
    create_input_field(form, 'boussole_succes_preuve', pos=(text_x, y_succes - 2.5*cm), size=(width - text_x - 1*cm, 2*cm),
...
def create_sac_a_dos_page(c):
    ...
    """
    layout = PageLayout(c, "Ce que je dépose aujourd'hui", config=LayoutConfig(part_title="1. Récapitulatif de la séance précédente"))
    layout.add_text("Allégeons le sac à dos. Je décide de déposer :", config=TextConfig(spacing_after=0.3*cm))
    
    layout.add_question_block("Je lâche cette croyance :", "sac_croyance", config=QuestionConfig(box_height=2.5*cm))
    layout.add_question_block("Je ne veux plus subir :", "sac_subir", config=QuestionConfig(box_height=2.5*cm))
    layout.add_question_block("Ma plus grande peur est :", "sac_peur", config=QuestionConfig(box_height=2.5*cm))
    
    layout.add_text("...et je décide de la regarder en face.", config=TextConfig(style_choice="italic"))
...
def create_heritage_page(c):
    ...
    """
    layout = PageLayout(c, "Mon Héritage (Matrice 3FVS)", config=LayoutConfig(part_title="2. Mes héritages"))
    layout.add_text("Identifiez ce que vous avez reçu pour décider de ce que vous en faites.", config=TextConfig(spacing_after=0.3*cm))
    
    layout.add_question_block("1. FORCES (Ce que je garde / Résilience)", "heritage_forces", 
                              config=QuestionConfig(box_height=3.5*cm, subtitle="Quelles qualités, valeurs ou savoir-faire de ma famille sont des atouts ?"))
                              
    layout.add_question_block("2. VIGILANCES (Ce que je laisse / Schémas)", "heritage_vigilances", 
                              config=QuestionConfig(box_height=3.5*cm, subtitle="Quels comportements ou croyances limitantes je décide de ne pas reproduire ?"))
                              
    layout.add_question_block("3. SOUHAITS & COMPTES (Mandats Familiaux)", "heritage_souhaits", 
                              config=QuestionConfig(box_height=3.5*cm, subtitle="Qu'est-ce qu'on voulait pour moi ? A qui ai-je l'impression de devoir quelque chose ?"))
...
def create_work_image_page(c):
    ...
    y_cursor -= 2.6*cm
    create_input_field(form, 'image_sensorielle', pos=(text_x, y_cursor), size=(width - text_x - 1.5*cm, 2.2*cm),
    ...
    create_input_field(form, 'image_mots_heritage', pos=(text_x, y_cursor - 2.5*cm), size=(col_width, 2.2*cm), multiline=True)
    ...
    create_input_field(form, 'image_mots_futur', pos=(right_col_x, y_cursor - 2.5*cm), size=(col_width, 2.2*cm), multiline=True)
...
def create_mentors_page(c):
    ...
    """
    layout = PageLayout(c, "Mentors & Anti-Modèles", config=LayoutConfig(part_title="2. Mes héritages"))
    layout.add_question_block("Mes Mentors (Inspirations)", "mentors_positif", 
                              config=QuestionConfig(box_height=6.0*cm, subtitle="Qui est votre héros professionnel (réel ou fictif) et pourquoi ? (J'admire X pour...)"))
                              
    layout.add_question_block("Mes Anti-Modèles (Repoussoirs)", "mentors_negatif", 
                              config=QuestionConfig(box_height=6.0*cm, subtitle="Quels sont les comportements ou situations que vous refusez de reproduire ? (Je ne veux pas reproduire...)"))
