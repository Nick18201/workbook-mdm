...
def create_psycho_edu_pages(c):
    ...
    new_y = draw_title(
        c, "Comprendre ses Racines", pos=(card_margin + 0.5 * cm, height - 2.5 * cm)
    )
    ...
    draw_side_panel(c, card_margin, width, height)
    new_y = draw_title(
        c, "Comprendre ses Racines (suite)", pos=(text_x, height - 2.5 * cm)
    )
    text_y = new_y - 1.5 * cm
    ...
    c.drawCentredString(
        center_x, text_y - 1.5 * cm, "En éclairant ces zones d'ombre, vous transformez"
    )
    c.drawCentredString(
        center_x, text_y - 2 * cm, "des chaînes invisibles en tremplins."
    )
